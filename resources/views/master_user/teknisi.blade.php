...
@section('title', 'Master Teknisi - Helpdesk')
...
<div class="container-fluid">
    ...
    <div class="card border-0 shadow-sm">
        <div class="card-header bg-white border-bottom d-flex justify-content-between align-items-center" style="padding: 20px;">
            ...
            <button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#addTeknisiModal">
                ...
                <table class="table table-hover mb-0">
                    ...
                    <tbody>
                        @forelse($teknisis as $teknisi)
                            @php
                                $initial = strtoupper(substr($teknisi->nama ?? 'T', 0, 1));
                                $avatarFallback = 'https://via.placeholder.com/40/667eea/ffffff?text=' . urlencode($initial);
                                $avatarSrc = $teknisi->photo ? asset('storage/' . $teknisi->photo) : $avatarFallback;
                            @endphp
                            <tr>
                                <td>{{ ($teknisis->currentPage() - 1) * $teknisis->perPage() + $loop->index + 1 }}</td>
                                <td>
                                    <img src="{{ $avatarSrc }}" alt="{{ $teknisi->nama }}" class="rounded-circle" 
                                         style="width: 40px; height: 40px; object-fit: cover; border: 2px solid #667eea;">
                                </td>
                                <td>{{ $teknisi->nik ?? '-' }}</td>
                                <td><strong>{{ $teknisi->username ?? '-' }}</strong></td>
                                <td>{{ $teknisi->nama ?? '-' }}</td>
                                <td>{{ $teknisi->email ?? '-' }}</td>
                                <td>
                                    <span class="badge bg-info">{{ $teknisi->departemen->nama_departemen ?? '-' }}</span>
                                </td>
                                <td>
                                    ...
                                    <button type="button" class="btn btn-sm btn-danger" 
                                            onclick="confirmDelete({{ $teknisi->id }}, '{{ $teknisi->nama }}')"
                                            title="Hapus">
                        ...
                        @empty
                            <tr>
                                <td colspan="8" class="text-center py-5">
                                    ...
                                    <span class="text-muted">Belum ada data teknisi</span>
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
...
<div class="modal fade" id="addTeknisiModal" tabindex="-1" data-bs-backdrop="static" data-bs-keyboard="false">
    <div class="modal-dialog modal-dialog-centered" style="max-width: 500px;">
        <div class="modal-content" style="background-color: white; border: 1px solid #e9ecef;">
            ...
            <form id="addTeknisiForm" enctype="multipart/form-data">
                @csrf
...
<!-- ============ MODAL EDIT TEKNISI ============ -->
<div class="modal fade" id="editTeknisiModal" tabindex="-1" data-bs-backdrop="static" data-bs-keyboard="false">
    <div class="modal-dialog modal-dialog-centered" style="max-width: 500px;">
        <div class="modal-content" style="background-color: white; border: 1px solid #e9ecef;">
            <div class="modal-header" style="background: white; border-bottom: 1px solid #e9ecef; padding: 20px;">
                <h5 class="modal-title" style="font-weight: 600; color: #2c3e50;">
                    <i class="fas fa-user-edit"></i> Edit Teknisi
                </h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" style="opacity: 0.5;"></button>
            </div>
            <form id="editTeknisiForm" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <input type="hidden" id="editTeknisiId">
                <div class="modal-body" style="padding: 20px;">
                    <!-- NIK -->
                    <div class="mb-3">
...
@section('scripts')
<script>
    const baseUrl = "{{ route('teknisi.index') }}";
    ...
    document.getElementById('addTeknisiModal').addEventListener('show.bs.modal', function() {
        clearFormErrors('add');
    });

    document.getElementById('editTeknisiModal').addEventListener('show.bs.modal', function() {
        clearFormErrors('edit');
    });

    // Handle form submit untuk TAMBAH
    document.getElementById('addTeknisiForm').addEventListener('submit', async (e) => {
        e.preventDefault();

        const formData = new FormData(e.target);
        
        try {
            const response = await fetch("{{ route('teknisi.store') }}", {
                method: 'POST',
                headers: {
            ...
            if (response.ok) {
                ...
                const modal = bootstrap.Modal.getInstance(document.getElementById('addTeknisiModal'));
                modal.hide();

                document.getElementById('addTeknisiForm').reset();
    ...
    // Function edit teknisi
    function editTeknisi(id) {
        fetch(`{{ url('teknisi') }}/${id}`)
            .then(response => {
                if (!response.ok) throw new Error('Data tidak ditemukan');
                return response.json();
            })
            .then(data => {
                document.getElementById('editTeknisiId').value = data.id;
    ...
    document.getElementById('editTeknisiForm').addEventListener('submit', async (e) => {
        e.preventDefault();

        const teknisiId = document.getElementById('editTeknisiId').value;
        const formData = new FormData(e.target);

        try {
            const response = await fetch(`{{ url('teknisi') }}/${teknisiId}`, {
                method: 'POST',
                headers: {
                    'X-CSRF-TOKEN': csrfToken,
                },
            ...
            if (response.ok) {
                ...
                const modal = bootstrap.Modal.getInstance(document.getElementById('editTeknisiModal'));
                modal.hide();

                document.getElementById('editTeknisiForm').reset();
                document.getElementById('editPhotoPreview').innerHTML = '';
    ...
    function confirmDelete(id, name) {
        if (confirm(`Apakah Anda yakin ingin menghapus teknisi "${name}"?`)) {
            deleteTeknisi(id);
        }
    }

    // Function delete teknisi
    ...
    function clearFormErrors(prefix) {
        document.querySelectorAll(`#${prefix}TeknisiForm .invalid-feedback`).forEach(el => {
            el.textContent = '';
