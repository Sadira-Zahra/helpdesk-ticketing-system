...
@section('title', 'Master Admin - Helpdesk')

@section('page-title', 'Master Admin')
                    ...
                    <tbody>
                        @forelse($admins as $admin)
                            ...
                            <tr>
                                ...
                                <td>{{ $admin->email ?? '-' }}</td>
                                <td>
                                    <span class="badge bg-secondary">{{ $admin->departemen->nama_departemen ?? '-' }}</span>
                                </td>
                                <td>
                        ...
                        @empty
                            <tr>
                                <td colspan="8" class="text-center py-5">
                                    ...
                                    <span class="text-muted">Belum ada data admin</span>
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
...
@section('scripts')
<script>
    const baseUrl = "{{ route('admin.index') }}";
    ...
    document.getElementById('editAdminForm').addEventListener('submit', async (e) => {
        ...
        try {
            const response = await fetch(`{{ url('admin') }}/${adminId}`, {
                method: 'POST',
                headers: {
                    'X-CSRF-TOKEN': csrfToken,
                },
